...
func (sess *session) Load(line string, env assert.Environment) {
...
func (sess *session) Delete(line string) {
	...
	case ROOT:
		xEnv.RemoveTask(line, assert.CONSOLE)
...
func (sess *session) Do(env assert.Environment) {
...
func (sess *session) DoString(line string, env assert.Environment) {
	co := env.Coroutine()
...
func (sess *session) handler(env assert.Environment) {
	defer sess.Close()
